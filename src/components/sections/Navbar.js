...
const NavContainer = styled.div`
  ...
  height: 100%;
  min-height: ${({ $scrolled }) => ($scrolled ? '64px' : '80px')};
...
const LogoImg = styled.img`
  ...
  filter: drop-shadow(0 0 8px rgba(0, 242, 254, 0.5))
          drop-shadow(0 0 15px rgba(0, 242, 254, 0.2));
  transition: transform 0.3s ease, filter 0.3s ease;

  &:hover {
    transform: scale(1.05);
    filter: drop-shadow(0 0 15px rgba(0, 242, 254, 0.85))
            drop-shadow(0 0 25px rgba(0, 242, 254, 0.45));
  }
...
export default function Navbar() {
  ...
  const links = [
    ...
    { label: t('nav.testimonials'), href: '#learning-journey' },
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['about', 'skills', 'achievements', 'certifications', 'my-projects', 'learning-journey', 'contact'];
