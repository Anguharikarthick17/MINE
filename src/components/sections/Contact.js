...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      const topicText = selectedChip ? `*Topic:* ${selectedChip}\n` : '';
      const whatsappText = `Hello! I contacted you from your portfolio website.\n\n*Name:* ${name}\n*Email:* ${email}\n${topicText}*Message:* ${message}`;
      const encodedText = encodeURIComponent(whatsappText);
      const whatsappUrl = `https://wa.me/918072522246?text=${encodedText}`;
  ...
  const copyEmail = () => {
    navigator.clipboard.writeText('anguharikarthick@gmail.com').catch(() => {});
    setCopiedEmail(true);
    setTimeout(() => setCopiedEmail(false), 2000);
  };

  const copyPhone = () => {
    navigator.clipboard.writeText('8072522246').catch(() => {});
    setCopiedPhone(true);
      ...
      <ContactContainer>
        ...
        <ContactLayout>
          ...
          <LeftPanel>
            ...
            <ContactInfoCards>
              <InfoCard onClick={copyEmail} $isRTL={isRTL}>
                ...
                <InfoCardContent>
                  ...
                  <InfoCardValue>
                    {copiedEmail ? <span style={{ color: '#10b981' }}>{t('contact.copiedSucc') || 'Copied ✓'}</span> : 'anguharikarthick@gmail.com'} 
                    {!copiedEmail && <CopyHint>{t('contact.copyHint') || 'click to copy'}</CopyHint>}
                  </InfoCardValue>
                </InfoCardContent>
              </InfoCard>

              <InfoCard onClick={copyPhone} $isRTL={isRTL}>
                <InfoCardIcon $color="#34d399">
                  <FiPhone />
                </InfoCardIcon>
                <InfoCardContent>
                  <InfoCardLabel>{t('contact.phone') || 'Phone'}</InfoCardLabel>
                  <InfoCardValue>
                    {copiedPhone ? <span style={{ color: '#10b981' }}>{t('contact.copiedSucc') || 'Copied ✓'}</span> : '8072522246'} 
                    {!copiedPhone && <CopyHint>{t('contact.copyHint') || 'click to copy'}</CopyHint>}
                  </InfoCardValue>
                </InfoCardContent>
              </InfoCard>
            ...
            <SocialsRow>
              <SocialBtn href="https://github.com/Anguharikarthick17" target="_blank" title="GitHub">
                <FaGithub />
              </SocialBtn>
              <SocialBtn href="https://www.linkedin.com/in/angu-hari-karthick-m-175753374/" target="_blank" title="LinkedIn">
                <FaLinkedinIn />
              </SocialBtn>
              <SocialBtn href="https://www.instagram.com/ahk.7teen/" target="_blank" title="Instagram">
                <FaInstagram />
              </SocialBtn>
            </SocialsRow>
          </LeftPanel>
